...
An agentic, AI-powered homelab Security Operations Center. Four autonomous agents —
**Red, Blue, Green, Purple** — each drive Claude via the CLI and coordinate through a small
Flask/SQLite messaging platform, running against an intentionally vulnerable lab network.

This is a **learning and portfolio project**, packaged as a template you can clone and adapt
to your own homelab. It is not production security tooling.

---

## ⚠️ Security — read before deploying

**This project runs autonomous agents with unrestricted shell access. Understand these
risks before pointing it at anything.**

### Prompt injection is an open, unmitigated risk

The agents invoke Claude with `--dangerously-skip-permissions`, giving them unrestricted
shell access on their host. Attacker-controlled fields from Wazuh alert bodies — User-Agent
strings, filenames, command lines — flow into agent prompts **without sanitization**. A
crafted log entry could, in principle, influence an agent that is able to run shell
commands.

This is a known architectural weakness in the reference design and is **not yet mitigated**.
Do not run this against real or untrusted traffic. Treat it as a lab exercise on an isolated,
segmented network. If you extend it, adding external input validation (rather than relying on
the model to police itself) should be your first priority.

### Other essentials

- **Runs an intentionally vulnerable VM** (Metasploitable 2) on purpose. Keep the lab
  network isolated.
- **Secrets live only in `.env`**, which is gitignored. The Discord webhook URLs are bearer
  credentials — anyone holding the URL can post to your server. Never commit them; rotate
  immediately if one leaks.
- **Agents run as a non-root user** by design. Keep it that way.
...
```bash
git clone <your-fork-url> ghost-lab && cd ghost-lab
