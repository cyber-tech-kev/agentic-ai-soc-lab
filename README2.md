...
The agents invoke claude with the '--dangerously-skip-permissions', this gives them unrestricted shell access on their respective host. Attacker-controlled fields from Wazuh alert bodies, User-Agent strings, filenames, and command lines flow into agent prompts **without sanitization**. A crafted log entry could in theory influence an agent that is able to run shell commands. (This has not been tested yet).
